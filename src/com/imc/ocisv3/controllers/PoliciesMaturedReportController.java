...
public class PoliciesMaturedReportController extends SelectorComposer<Window>{
	
	private Logger log = LoggerFactory.getLogger(PoliciesMaturedReportController.class);
	...
	private void populateActiveMember() {
		...
		try{
			/*String query = "SELECT HLTINS.HINSNAME, HLTINS.HINSID, COUNT(1) as member "
					     ...
					     + "AND HLTDT2.HDT2CTR=HLTEMP.HEMPCTR AND HLTDT2.hdt2moe NOT in('M','U','C','D') "
					     + "GROUP BY HLTINS.HINSNAME, HLTINS.HINSID ORDER BY HLTINS.HINSNAME";*/
			
			String query = "select convert(varchar, hhdryy)+'-1-0-'+convert(varchar,hhdrpono) as NoPolis, hhdrname, hinsname, "
					     + "convert(varchar,hhdrmdtyy)+'-'+convert(varchar,hhdrmdtmm)+'-'+convert(varchar,hhdrmdtdd) as matureDate from IDNHLTPF.dbo.HLTHDR hlthdr "
					     + "inner join IDNHLTPF.dbo.HLTINS hltins on hlthdr.hhdrinsid=hltins.hinsid where "
					     + "DATEDIFF(day,getdate(),convert(datetime,convert(varchar,hhdrmdtmm)+'-'+convert(varchar, hhdrmdtdd)+'-'+convert(varchar,hhdrmdtyy), 110)) "
					     + "between 0 and 31 and hhdrpono<>99999 ";
